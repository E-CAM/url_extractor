(function ($, Configuration) {
    ...
    $.when(extractor_req, featherlight_req).done(function(extract_data, featherlight_data){
        ...
        $(Configuration.tab).append("<h1 align='center'><a href='" + fullurl + "' target='_blank'>" + fullurl + "</a></h1>");

        $(Configuration.tab).append("<a href='" + fullurl + "' id='mypreviewlink'><img class='fit-in-space' src='" + preview + "'/></a>");

        $('#mypreviewlink').featherlight(fullurl, {type:'iframe', loading: 'Loading ' + fullurl, iframeWidth: $(window).width() * 0.8, iframeHeight: $(window).height() * 0.8});
